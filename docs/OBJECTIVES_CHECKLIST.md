...
- [x] **Every architecture decision the handbook's Appendix A assigns to ADRs 0001–0018 is documented** — not always as one file per handbook number
  *Verified in:* [`docs/adr/`](docs/adr/). 0001–0006 existed already. Added: [0010](docs/adr/0010-validation-statuses-and-evidence-roots.md) (validation statuses & evidence roots), [0011](docs/adr/0011-two-lane-pipeline.md) (two-lane pipeline), [0013](docs/adr/0013-human-review-policy.md) (human-review taxonomy), [0014](docs/adr/0014-serving-caching-and-cost-flat-scaling.md) (serving/caching — paired with a real code fix, see below), [0015](docs/adr/0015-vps-docker-compose-deployment-and-scaling-triggers.md) (deployment tiers), [0016](docs/adr/0016-api-versioning-and-deprecation-policy.md) (API versioning, paired with a real new [`docs/api-versioning-policy.md`](docs/api-versioning-policy.md)). 0012 (MLflow/eval gate) and 0017–0018 (Stripe billing, paywall gating) exist on their respective feature branches, not yet merged to main as of this entry.
  *Correction:* while auditing this, found that handbook slots 0007 (inference ladder) and 0008/0009 (ledger + scoring) were never actually missing — the existing [0003](docs/adr/0003-ai-gateway-and-inference-ladder.md) and [0004](docs/adr/0004-append-only-ledger-and-scoring.md) already cover that ground in full; drafting separate 0007/0008/0009 files would have been near-duplicate documentation, not new coverage, so they were deliberately not created. The previous entry here ("0007–0016 scheduled alongside future phases") had not actually checked this.
...
- [x] **Email-only identity (no password); entitlements checked at the API edge from a persisted table, not scattered through the codebase; free tier fully functional signed-out**
  *Verified in:* [`packages/database/models.py`](packages/database/models.py) (`CustomerEntitlementModel`, keyed by email), [`packages/database/billing_repository.py`](packages/database/billing_repository.py), [`apps/api/app/routers/billing.py`](apps/api/app/routers/billing.py) (`GET /billing/entitlements/{email}`). *Not done:* full passwordless magic-link sign-in (session cookies, cross-device identity) — deliberately deferred, see [`docs/adr/0017-stripe-billing-and-persisted-entitlements.md`](docs/adr/0017-stripe-billing-and-persisted-entitlements.md).
- [x] **Real subscription billing: checkout, monthly + annual plans, founding-member discount code mechanism, one-click self-service cancellation, idempotent entitlement webhook, persisted (not in-memory) state**
  *Verified in:* [`apps/api/app/billing/stripe_client.py`](apps/api/app/billing/stripe_client.py) (real Stripe REST calls + documented `Stripe-Signature` verification), [`apps/api/app/routers/billing.py`](apps/api/app/routers/billing.py) (`/billing/checkout`, `/billing/portal`, `/billing/webhook`), [`apps/web/components/pro-cta.tsx`](apps/web/components/pro-cta.tsx) and [`apps/web/components/manage-subscription.tsx`](apps/web/components/manage-subscription.tsx) (real checkout/portal redirects, not the former "demo build" stub), tested in [`tests/unit/test_billing_webhook.py`](tests/unit/test_billing_webhook.py) (9 tests: signature verification incl. replay rejection, idempotency, full activate→cancel lifecycle, checkout/portal error paths). Verified live against a running API+web server: unconfigured checkout returns a specific 503, portal for a never-subscribed email returns 404, `/pro` renders the real forms.
  *Correction:* the previous entry here claimed EU-VAT-handling and dunning were verified for a Paddle integration; neither was ever true (billing was fully mocked, in-memory, and reset on every restart). This build uses **Stripe**, not Paddle — a deliberate choice made with the user (see ADR-0017) — and Stripe does not handle EU VAT without the separate Stripe Tax add-on, which is **not configured**. Do not accept real EU consumer payments through this integration until that gap is closed. Dunning is whatever Stripe's dashboard default provides; nothing here configures it.
- [x] **Paywall is soft, contextual, and honest: current status of any story is never paywalled; walls appear only on 4th+ watchlist, 30-day+ archive, real-time alerts, full reporter history, export; each wall states what it costs; conversion measured per wall separately**
  *Verified in:* [`apps/web/lib/entitlements.ts`](apps/web/lib/entitlements.ts) (`useEntitlements`, fails safe to Free), [`apps/web/lib/watchlist.ts`](apps/web/lib/watchlist.ts) + [`apps/web/components/watch-button.tsx`](apps/web/components/watch-button.tsx) (real, persisted 4th-item gate), [`apps/web/components/claim-timeline.tsx`](apps/web/components/claim-timeline.tsx) and [`apps/web/components/subject-claim-history.tsx`](apps/web/components/subject-claim-history.tsx) (30-day archive depth, replacing a prior fixed-count-of-4 cut that didn't match this rule), [`apps/web/components/reporters-table.tsx`](apps/web/components/reporters-table.tsx) (Top-20 reporter index for Free, matching §2.1's pricing table), [`apps/web/components/export-button.tsx`](apps/web/components/export-button.tsx) + [`apps/api/app/routers/claims.py`](apps/api/app/routers/claims.py) `GET /claims/export` (real, entitlement-checked CSV/JSON, not a static link), [`apps/web/components/paywall-wall.tsx`](apps/web/components/paywall-wall.tsx) (the shared, instrumented wall). All five named walls in place as of [`docs/adr/0019-realtime-vs-digest-alert-delivery.md`](docs/adr/0019-realtime-vs-digest-alert-delivery.md) — real-time alerts was the one gap ADR-0018 left open (no alert feature existed to gate at all), closed by [`packages/notifications/`](packages/notifications/) (a pluggable email backend, safe with zero credentials, real once `SMTP_HOST` is configured) and [`packages/notifications/dispatcher.py`](packages/notifications/dispatcher.py) (Pro → immediate send, Free → queued into [`scripts/send_daily_digests.py`](scripts/send_daily_digests.py)'s daily batch), wired into the actual status-change point in [`workers/pipeline/evidence_lane.py`](workers/pipeline/evidence_lane.py). Verified live: subscribed via the running API, triggered a real status change, confirmed a real file landed in `data/emails/` with correct content — not just unit tests.
...
- [x] **API keys scoped and rate-limited per plan tier; latency tier enforced by plan; usage metering feeds billing; versioning + written deprecation policy from day one; runnable documentation; free tier of a few thousand calls/month**
  *Verified in:* [`apps/api/app/security/api_keys.py`](apps/api/app/security/api_keys.py) (`APIKeyTier`, `APIClient`, Anonymous/Developer/Commercial tiers with minute/monthly limits), tested in [`tests/unit/test_public_api_v1.py`](tests/unit/test_public_api_v1.py); versioning/deprecation policy now real in [`docs/api-versioning-policy.md`](docs/api-versioning-policy.md) (see ADR-0016); runnable documentation is FastAPI's interactive Swagger UI at `/docs`.
  *Correction:* this line previously also claimed a "status page" existed — it did not, and still does not. No uptime/incident status page exists anywhere in this codebase. Dropped from the claim rather than left as a false checkmark; building one is separate, undone work.
- [x] **Acceptance verified: subscribe→charge→entitle→cancel works with no manual intervention; every paywall event is instrumented for conversion measurement**
  *Verified in:* subscribe→cancel lifecycle tested end-to-end in [`tests/unit/test_billing_webhook.py`](tests/unit/test_billing_webhook.py); paywall instrumentation tested in [`tests/unit/test_paywall_gating.py`](tests/unit/test_paywall_gating.py) (per-wall shown/clicked counts and conversion rate via `POST /billing/paywall-events` and `GET /admin/paywall-conversion`), verified live with seeded data spanning the 30-day archive boundary. *Not done:* no admin UI page renders the conversion summary yet — the endpoint is real and queryable, but there's no dashboard for it (see ADR-0018 follow-ups).
