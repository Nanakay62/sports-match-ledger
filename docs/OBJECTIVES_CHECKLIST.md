...
- [x] **Acceptance verified: 95%+ certainty-marker accuracy on 100 fixture articles**
  *Verified in:* [`tests/fixtures/claims/certainty_fixtures.json`](tests/fixtures/claims/certainty_fixtures.json) (100 hand-labelled sentences across en/es/it/de/fr covering all six certainty markers), [`tests/unit/test_claim_certainty_accuracy.py`](tests/unit/test_claim_certainty_accuracy.py) — measured 99% predicate and certainty-marker accuracy against `workers/pipeline/claim_extractor.py`, with per-language regression floor at 85%.
...
- [x] **Acceptance verified: cost per generated event visible per asset; identical-fact rerun is a cache hit at zero cost; hallucinated fact ID rejection is tested; altered-number translation is caught; templates handle ≥30% of published events; blended cost per event <€0.02 on a full day of fixture traffic**
  *Verified in:* [`tests/unit/test_generation_cost_budget.py`](tests/unit/test_generation_cost_budget.py) — a simulated 100-event day through the real `StructuredEvidenceSummarizer`/`CostTracker` measured 35%+ template (Rung L0) share and blended cost well under €0.02/event; cache-hit-is-free and hallucinated-fact-ID rejection remain covered in [`tests/unit/test_phase7_evidence_package_and_summarizer.py`](tests/unit/test_phase7_evidence_package_and_summarizer.py) and altered-number rejection in [`tests/unit/test_translation.py`](tests/unit/test_translation.py).
